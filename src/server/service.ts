...
export async function ClientInfoServiceGet(
  ...
  if (allData.length === 0) {
    // NOTE generates all data for the frotend
    for (let i = 1; i <= 1000000; i++) {
      // algo that makes from 1 to 1000000
  ...
  if ((!order || order.length === 0) && savedCustomOrder.length > 0)
    order = savedCustomOrder;
  ...
  return {
    // return all required info for the get request to frontend
    pageData: pageData,
...
export async function ClientInfoServicePost(
  body: any,
): Promise<{ ok: boolean }> {
